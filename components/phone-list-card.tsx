...
interface PhoneListCardProps {
  phone: PhoneSpec;
  onCompare?: (phoneId: string) => void;
}

export function PhoneListCard({ phone, onCompare }: PhoneListCardProps) {
  return (
    <div className="bg-white rounded border hover:shadow-sm p-2 flex items-center gap-3">
      <div className="w-14 h-14 bg-gray-100 rounded flex-shrink-0 flex items-center justify-center">
        <span className="text-xs text-gray-400">IMG</span>
      </div>
      
      <div className="flex-1 min-w-0">
        <h3 className="font-semibold text-sm truncate">{phone.brand} {phone.model}</h3>
        <div className="grid grid-cols-2 gap-x-3 gap-y-0.5 text-xs text-gray-600 mt-1">
          <div>
      ...
      <div className="flex flex-col gap-1.5 flex-shrink-0">
        ...
        <Link href={`/${phone.brand.toLowerCase()}/${phone.model.toLowerCase().replace(/\s+/g, '-')}`}>
          <Button
            variant="default"
            size="sm"
            className="h-7 text-xs px-3 w-full bg-[#4169E1] hover:bg-[#4169E1]/90"
          >
            Details
          </Button>
        </Link>
