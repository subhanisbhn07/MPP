    ...
    <Card className="group flex flex-col overflow-hidden transition-all duration-300 hover:shadow-lg h-full">
       <Link href={phoneUrl} className="p-4">
        <div className="relative">
            <div className="aspect-square w-full h-full overflow-hidden rounded-md">
                <Image
                src={phone.image}
            ...
            <Button 
                variant="ghost" 
                size="icon" 
                className="absolute top-0 right-0 h-8 w-8 bg-background/50 hover:bg-background/80 rounded-full"
      ...
      <div className="flex flex-col flex-1 p-4 pt-0 justify-between">
        <div>
            ...
            <div className="grid grid-cols-2 gap-2 mt-4 text-xs">
                ...
                <div className="flex-1 bg-muted/50 rounded-lg p-2 text-center flex flex-col justify-center items-center">
                    <Cpu className="h-5 w-5 mx-auto mb-1 text-primary" />
                    <p className="text-xs font-bold leading-tight">{phone.specs.platform.chipset.split('(')[0].trim()}</p>
                    <p className="text-[10px] text-muted-foreground">Chipset</p>
                </div>
            </div>
        </div>
        <div className="flex items-center justify-between mt-4">
            <p className="text-xl font-bold text-primary flex-shrink-0">${phone.price}</p>
