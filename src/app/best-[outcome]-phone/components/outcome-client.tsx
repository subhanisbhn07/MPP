...
import { ChevronDown, ChevronUp, ArrowRight, Trophy, Zap, Target } from 'lucide-react';
...
interface OutcomeClientProps {
  config: OutcomeConfig;
  phones: Phone[];
  faqs: { question: string; answer: string }[];
  relatedOutcomes: OutcomeConfig[];
  allOutcomes: OutcomeConfig[];
}

type SortOption = 'relevance' | 'price-asc' | 'price-desc' | 'latest' | 'rating';

export function OutcomeClient({ config, phones, faqs, relatedOutcomes, allOutcomes }: OutcomeClientProps) {
  ...
  const breadcrumbItems = [
    ...
    { label: `Best ${config.title} Phone`, href: `/best-${config.slug}-phone` },
  ];
      ...
      <div className="container mx-auto py-8 px-4">
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <h1 className="text-3xl md:text-4xl font-bold">
                Best {config.title} Phone ({currentYear})
              </h1>
              <p className="text-muted-foreground mt-1">
                {phones.length} phones ranked for {config.title.toLowerCase()}
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            <Card>
              <CardContent className="p-4 text-center">
                ...
                <p className="text-sm text-muted-foreground">Phones Ranked</p>
              </CardContent>
            </Card>
            <Card>
              <CardContent className="p-4 text-center">
        ...
        <div className="flex flex-col lg:flex-row gap-8">
              ...
              <Card>
                <CardContent className="p-8 text-center">
                  <p className="text-muted-foreground">
                    No phones found matching the criteria for best {config.title.toLowerCase()}.
                  </p>
                </CardContent>
              </Card>
            ) : (
          ...
          <aside className="lg:w-80 space-y-6">
            ...
            <Card>
              <CardHeader>
                <CardTitle className="text-lg">All Outcomes</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="flex flex-wrap gap-2">
                  {allOutcomes.map(outcome => (
                    <Link key={outcome.slug} href={`/best-${outcome.slug}-phone`}>
                      <Badge
                        variant={outcome.slug === config.slug ? 'default' : 'outline'}
            ...
            <Card>
              ...
              <CardContent className="space-y-2">
                ...
                <Link
                  href="/best-phone-for-photography"
                  className="flex items-center justify-between p-2 rounded-md hover:bg-muted transition-colors"
                >
                  <span>Best for Photography</span>
                  <ArrowRight className="h-4 w-4 text-muted-foreground" />
                </Link>
                <Link
                  href="/categories"
